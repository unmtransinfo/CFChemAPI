...
Date: 12/17/2025
Description:
Functional tests for lincs blueprint endpoints.
...
class TestLincs:
    ...
    def test_valid_mol_id_lookup(self, test_client, url_prefix):
        """
        GIVEN a Flask application configured for testing
        WHEN the '/lincs/{mol_id}' endpoint is requested with a valid mol_id
        THEN check that a valid object is returned with correct keys
        """
        mol_id = 20375  # genistein
        url = f"{url_prefix}/lincs/{mol_id}"
        response = test_client.get(url)
    ...
    def test_invalid_mol_id_lookup(self, test_client, url_prefix):
        """
        GIVEN a Flask application configured for testing
        WHEN the '/lincs/{mol_id}' endpoint is requested with an invalid mol_id
    ...
    def test_default_limit_behavior(self, test_client, url_prefix):
        """
        GIVEN a Flask application configured for testing
        WHEN the '/lincs' endpoint is requested without a limit parameter
        THEN check that the default limit (10) is applied
        """
        url = f"{url_prefix}/lincs/"
        response = test_client.get(url)
    ...
    def test_limit_edge_case_zero(self, test_client, url_prefix):
        """
        GIVEN a Flask application configured for testing
        WHEN the '/lincs' endpoint is requested with limit=0
        THEN check that no objects are returned
        """
        url = f"{url_prefix}/lincs/?limit=0"
        response = test_client.get(url)
    ...
    def test_limit_edge_case_one(self, test_client, url_prefix):
        """
        GIVEN a Flask application configured for testing
        WHEN the '/lincs' endpoint is requested with limit=1
        THEN check that exactly one object is returned
        """
        url = f"{url_prefix}/lincs/?limit=1"
        response = test_client.get(url)
    ...
    def test_offset_parameter(self, test_client, url_prefix):
        """
        GIVEN a Flask application configured for testing
        WHEN the '/lincs' endpoint is requested with offset parameter
        ...
        url_first = f"{url_prefix}/lincs/?limit={limit}"
        ...
        url_offset = f"{url_prefix}/lincs/?limit={limit}&offset={offset}"
    ...
    def test_response_format_is_json(self, test_client, url_prefix):
        """
        GIVEN a Flask application configured for testing
        WHEN the '/lincs' endpoint is requested
    ...
    def test_large_limit_value(self, test_client, url_prefix):
        """
        GIVEN a Flask application configured for testing
        WHEN the '/lincs' endpoint is requested with a large limit value
